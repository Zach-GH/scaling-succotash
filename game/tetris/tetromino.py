...
A file that handles block creation, action, and interactions with other
objects.  This includes block rotation, spinning the blocks when a line clears
block on block collision, and block grid landing.
"""
...
class Block(pg.sprite.Sprite):
    ...
    def sfx_run(self):
        ...
        self.image = pg.transform.rotate(self.image,
                                         pg.time.get_ticks() * self.sfx_speed)
    ...
    def is_alive(self):
        if not self.alive:
            if not self.sfx_end_time():
                ...
                if self.tetromino.tetris.music == True:
                    sfx_sound.play()
                self.kill()
...
class Tetromino:
    ...
    def move(self, direction):
        ...
        new_block_positions = [block.pos + move_direction 
